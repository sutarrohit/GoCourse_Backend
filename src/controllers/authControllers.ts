...
// Signup verification
export const userVerification = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const hashedToken = crypto
      .createHash("sha256")
      .update(req.params.token)
...
export const protect = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!currentUser)
      throw new AppError("The User Belongs To This Token No Longer Exits", 401);
...
export const protectAdmin = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    if (req.user.role !== "admin")
      throw new AppError("You are not authorized to access admin routes", 401);
    next();
  }
);
